...
int main(int argc, char *argv[]) {
    ...
    if (size < 1) {
        ...
        a = (float *)clSVMAlloc(context, CL_MEM_READ_WRITE, size * size * sizeof(float), 0);
/*malloc(size * size * sizeof(float));*/
        cl_SVMMap((void*)a, size * size * sizeof(float), CL_MAP_WRITE_INVALIDATE_REGION);
        InitMat(fp, size, a, size, size);
        cl_SVMUnmap((void*)a);
        b = (float *)clSVMAlloc(context, CL_MEM_READ_WRITE, size * sizeof(float), 0);
/*malloc(size * sizeof(float));*/
        cl_SVMMap((void*)b, size * sizeof(float), CL_MAP_WRITE_INVALIDATE_REGION);
        InitAry(fp, b, size);
        cl_SVMUnmap((void*)b);
        fclose(fp);
...
/*malloc(size * sizeof(float));*/
    ...
    m = (float *)clSVMAlloc(context, CL_MEM_READ_WRITE, size * size * sizeof(float), 0);
/*malloc(size * size * sizeof(float));*/

    // create a new vector to hold the final answer

    finalVec = (float *)malloc(size * sizeof(float));
/*clSVMAlloc(context, CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER, size * sizeof(float), 0);*/
/*malloc(size * sizeof(float));*/
    cl_SVMMap((void*)m, size*size*sizeof(float), CL_MAP_WRITE_INVALIDATE_REGION);
    InitPerRun(size, m);
    cl_SVMUnmap((void*)m);
    ...
    BackSub(a, b, finalVec, size);
    char outputName[20];
    sprintf(outputName, "out_%d.bin", size);
...
void ForwardSub(cl_context context, float *a, float *b, float *m, int size,
    ...
    gaussianElim_program =
        cl_compileProgram((char *)"gaussianElim_kernels.cl", "-cl-std=CL2.0");
...
/*
    ...
    b_dev = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * size,
                           NULL, &error);
...
*/
    ...
    if (localWorksizeFan1Buf[0]) {
        ...
        globalWorksizeFan1[0] = (int)ceil(globalWorksizeFan1[0] /
                                          (double)localWorks\
izeFan1Buf[0]) * localWorksizeFan1Buf[0];
    }
    if (localWorksizeFan2Buf[0]) {
        localWorksizeFan2 = localWorksizeFan2Buf;
        globalWorksizeFan2[0] = (int)ceil(globalWorksizeFan2[0] /
                                          (double)localWorks\
izeFan2Buf[0]) * localWorksizeFan2Buf[0];
        globalWorksizeFan2[1] = (int)ceil(globalWorksizeFan2[1] /
                                          (double)localWorks\
...
/*        argchk = clSetKernelArg(fan1_kernel, 0, sizeof(cl_mem), (void *)&m_dev);
        argchk |=
            clSetKernelArg(fan1_kernel, 1, sizeof(cl_mem), (void *)&a_dev);
        argchk |=
            clSetKernelArg(fan1_kernel, 2, sizeof(cl_mem), (void *)&b_dev);
*/
        argchk = clSetKernelArgSVMPointer(fan1_kernel, 0, (void*) m);
        argchk |= clSetKernelArgSVMPointer(fan1_kernel, 1, (void*) a);
        argchk |= clSetKernelArgSVMPointer(fan1_kernel, 2, (void*) b);
...
/*        argchk = clSetKernelArg(fan2_kernel, 0, sizeof(cl_mem), (void *)&m_dev);
        argchk |=
            clSetKernelArg(fan2_kernel, 1, sizeof(cl_mem), (void *)&a_dev);
        argchk |=
            clSetKernelArg(fan2_kernel, 2, sizeof(cl_mem), (void *)&b_dev);
*/
        argchk = clSetKernelArgSVMPointer(fan2_kernel, 0, (void*) m);
        argchk |= clSetKernelArgSVMPointer(fan2_kernel, 1, (void*) a);
        argchk |= clSetKernelArgSVMPointer(fan2_kernel, 2, (void*) b);
        ...
        error = clEnqueueNDRangeKernel(command_queue, fan2_kernel, 2, 0,
                                       globalWorksizeFan2, NULL, 0, NULL,
                                       &kernelEvent);
...
void BackSub(float *a, float *b, float *finalVec, int size) {
    cl_SVMMap((void*)a, size * size * sizeof(float), CL_MAP_READ);
    cl_SVMMap((void*)b, size * sizeof(float), CL_MAP_READ);
    // solve "bottom up"
    int i, j;
    for (i = 0; i < size; i++) {
